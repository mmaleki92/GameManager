...
class CameraGroup(pygame.sprite.Group):
    ...
    def mouse_control(self):
        ...
        self.mouse_offset_vector = pygame.math.Vector2()
        ...
        if top_border < mouse.y < bottom_border:
            if mouse.x < left_border:
                self.mouse_offset_vector.x = mouse.x - left_border
                pygame.mouse.set_pos((left_border, mouse.y))
            if mouse.x > right_border:
                self.mouse_offset_vector.x = mouse.x - right_border
                pygame.mouse.set_pos((right_border, mouse.y))
        elif mouse.y < top_border:
            if mouse.x < left_border:
                self.mouse_offset_vector = mouse - pygame.math.Vector2(left_border, top_border)
                pygame.mouse.set_pos((left_border, top_border))
            if mouse.x > right_border:
                self.mouse_offset_vector = mouse - pygame.math.Vector2(right_border, top_border)
                pygame.mouse.set_pos((right_border,top_border))
        elif mouse.y > bottom_border:
            if mouse.x < left_border:
                self.mouse_offset_vector = mouse - pygame.math.Vector2(left_border, bottom_border)
                pygame.mouse.set_pos((left_border, bottom_border))
            if mouse.x > right_border:
                self.mouse_offset_vector = mouse - pygame.math.Vector2(right_border, bottom_border)
                pygame.mouse.set_pos((right_border, bottom_border))

        if left_border < mouse.x < right_border:
            if mouse.y < top_border:
                self.mouse_offset_vector.y = mouse.y - top_border
                pygame.mouse.set_pos((mouse.x, top_border))
            if mouse.y > bottom_border:
                self.mouse_offset_vector.y = mouse.y - bottom_border
                pygame.mouse.set_pos((mouse.x, bottom_border))

        self.offset += self.mouse_offset_vector * self.mouse_speed
    ...
    def camera_type(self, player):
        if "cneter_target" in self.camera_types:
            self.center_target_camera(player)
        
        if "box_target" in self.camera_types:
            self.box_target_camera(player)
        
        if "keyboard_control" in self.camera_types:
            self.keyboard_control()
        
        if "mouse_control" in self.camera_types:
            self.mouse_control()
